...
int main(int argc, char* argv[]){
    ...
    if (proc_num == 1){
        ...
        //            printf("Sending u[%d][%d] to proc %d\n", m_end - 1, p, proc_rank + 1);
                }
            ...
            for(unsigned p = 0; p < P; p++){
                if (p == 0){
        //            printf("Recieving u[%d][%d] from proc %d\n", m_begin - 1, p + 1, proc_rank - 1);    
                    if (p != P - 1) 
                        MPI_Irecv(&u[m_begin-1][p + 1], 1, MPI_DOUBLE, proc_rank - 1, MAT, MCW, &request_from);
                    for (unsigned m = m_begin - 1; m < m_end; m++){
                        step(u, m, p, x_step, t_step);
                    }
                    if (p != P - 1)
...
void step(double **u, unsigned m, unsigned p, double x_step, double t_step){
    if (m == 0){
        ...
        u[m][p] = (2 * f(m + 0.5, p-0.5, x_step, t_step) * t_step *x_step 
                - u[m - 1][p] * (x_step - t_step) 
                + u[m - 1][p - 1] * (x_step + t_step)
                + u[m][p - 1] * (x_step - t_step)) / (x_step + t_step);
}
